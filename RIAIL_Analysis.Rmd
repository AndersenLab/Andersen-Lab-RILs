...
QX_compare <- readr::read_tsv("QX_crossobj_genotypes.tsv") %>%
...
QX_summary <- QX_compare %>%
  dplyr::summarize(n = n(), freq_correct = mean(concordant)) %>%
  dplyr::mutate(index = as.double(stringr::str_extract(strain, "[0-9]+")))

QX_compare_old <- readr::read_tsv("QX_crossobj_genotypes_old.tsv") %>%
  tidyr::gather(strain, illumina, -CHROM, -POS) %>%
  dplyr::mutate(illumina = ifelse(illumina %in% c(1,2), illumina, NA)) %>%
  dplyr::left_join(gt) %>%
  dplyr::filter(complete.cases(.)) %>%
...
QX_summary_old <- QX_compare_old %>%
  dplyr::summarize(n = n(), freq_correct = mean(concordant)) %>%
  dplyr::mutate(index = as.double(stringr::str_extract(strain, "[0-9]+")))

conc_old <- readr::read_tsv("QX_concordance_old.tsv") %>%
            dplyr::rename(freq_correct_prev = freq_correct)

ggplot(QX_summary) +
  geom_point(aes(x=reorder(strain, freq_correct), y=freq_correct), stat="identity") +
  coord_flip() +
  theme(strip.background = element_blank(),
        legend.position = "None",
